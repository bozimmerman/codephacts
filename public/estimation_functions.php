...
function calculatePutnam($loc, $technology = 8000)
{
    ...
    $people = $timeMonths > 0 ? $effortMonths / $timeMonths : 0;
    return [
        'effort' => $effortMonths,
        'time' => $timeMonths,
        'people' => $people,
...
function generateEstimates($loc, $primaryLanguage = 'PHP', $cocomoMode = 'semi-detached')
{
    if ($loc <= 0) {
        return [
            'cocomo' => calculateCOCOMO(0, $cocomoMode),
            'cocomo2' => calculateCOCOMO2(0),
            'functionPoints' => calculateFunctionPoints(0, $primaryLanguage),
            'slim' => calculateSLIM(0),
            'putnam' => calculatePutnam(0)
        ];
    }
